	...
	private _stopProfiling(): void {
		...
		markerFile.then(() => {
			...
			return this._dialogService.confirm({
				...
				acceptButton: localize('prof.restartAndFileIssue', "&&Create Issue and Restart"),
				rejectButton: localize('prof.restart', "&&Restart")
			}).then(res => {
				if (res.confirmed) {
					Promise.all<any>([
						...
						return this._dialogService.confirm({
							...
							acceptButton: localize('prof.restart.button', "&&Restart"),

...
export class UserDataSyncWorkbenchService extends Disposable implements IUserDataSyncWorkbenchService {
	...
	private async onBeforeShutdown(manualSyncTask: IManualSyncTask): Promise<boolean> {
		const result = await this.dialogService.confirm({
			...
			acceptButton: localize({ key: 'yes', comment: ['&& denotes a mnemonic'] }, "&&Yes"),
			rejectButton: localize({ key: 'no', comment: ['&& denotes a mnemonic'] }, "&&No"),
		});
		if (result.confirmed) {
	...
	async resetSyncedData(): Promise<void> {
		const result = await this.dialogService.confirm({
			...
			type: 'info',
			acceptButton: localize({ key: 'resetButton', comment: ['&& denotes a mnemonic'] }, "&&Reset"),
		});
		if (result.confirmed) {
